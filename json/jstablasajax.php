...
switch ($tabla) {
	case 'lloguers':
		...
		if ($_SESSION['idlocatario_sahilices'] == '') {
			$resAjax = $serviciosReferencias->traerLloguersajax($length, $start, utf8_encode($busqueda),$colSort,$colSortDir);
			$res = $serviciosReferencias->traerLloguers();
			$termina = 7;
		} else {
			$resAjax = $serviciosReferencias->traerLloguersLocatarioajax($length, $start, utf8_encode($busqueda),$colSort,$colSortDir,$_SESSION['idlocatario_sahilices']);
